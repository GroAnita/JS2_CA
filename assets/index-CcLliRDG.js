...
/>
  ...
  class="
    ...
    >spell</button>
  `}const Se="cKEP0soLY46mO4r3od49mv8igrBEx40SIxANJlGmqIo",D=new URL("/JS2_CA/assets/storiesplaceholder-BEkCxyvR.png",import.meta.url).href;let R=null,Y=new Map;async function W(){if(R)return R;try{const t=await fetch("https://api.unsplash.com/photos/random?query=portrait fantasy portrait face&orientation=squarish&content_filter=high&client_id=cKEP0soLY46mO4r3od49mv8igrBEx40SIxANJlGmqIo");if(!t.ok)throw new Error;return R=(await t.json())?.urls?.small||D,R}catch{return D}}async function ue(e,t="fantasy"){if(Y.has(e))return Y.get(e);try{const n=await fetch(`https://api.unsplash.com/photos/random?query=${encodeURIComponent(t)}&orientation=landscape&content_filter=high&client_id=${Se}`);if(!n.ok)throw new Error;const a=(await n.json())?.urls?.regular||D;return Y.set(e,a),a}catch{return D}}let me=[];function X(e){me=e||[]}function U(){return me}async function te(){const e=U(),t=document.createElement("aside");t.className="hidden md:flex w-64 flex-col bg-[#121212] px-6 py-8 border-l border-gray-700";const n=document.createElement("h1");n.className="font-[Cinzel] mb-10 text-xl mx-auto font-semibold text-purple-400",n.textContent="Familiars",t.appendChild(n);const o=document.createElement("section");o.className="flex flex-col items-center gap-4 px-6 py-4 overflow-x-auto border-b border-gray-700 bg-[#121212]";const a=e.slice(0,6);for(const r of a){const l=r.avatar?.url||await W(),s=document.createElement("a");s.href=`/profiles/${r.name}`,s.dataset.link="",s.className="block";const i=document.createElement("div");i.className="flex flex-col items-center gap-3 shrink-0";const u=document.createElement("div");u.className="w-24 h-24 rounded-full border-2 border-purple-400 p-0.5 hover:border-purple-800 transition mb-4 mt-6";const p=document.createElement("img");p.src=l,p.alt=`${r.name} avatar`,p.className="w-full h-full rounded-full object-cover";const h=document.createElement("span");h.className="text-xs font-[Cinzel] text-gray-300",h.textContent=r.name,u.appendChild(p),i.appendChild(u),i.appendChild(h),s.appendChild(i),o.appendChild(s)}return t.appendChild(o),t}let A=null;const Q="authUser";function pe(e){A=e,localStorage.setItem(Q,JSON.stringify(e)),document.dispatchEvent(new Event("auth:changed"))}function ke(){A=null,localStorage.removeItem(Q),document.dispatchEvent(new Event("auth:changed"))}function ee(){if(A)return A;const e=localStorage.getItem(Q);if(!e)return null;try{return A=JSON.parse(e),A}catch{return null}}function z(){return!!fe()}function F(){const e=ee();return e?{username:e.name,email:e.email}:null}function fe(){return ee()?.accessToken||null}function $e(e){return e.message==="Failed to fetch"?"Network error: Please check your internet connection and try again.":e.message.includes("API error")?"Server error: Something went wrong on our end. Please try again later.":e.message.includes("Registration error")?"Registration failed: Please ensure all fields are filled out correctly and try again.":e.message.includes("Login error")?"Login failed: Incorrect username or password. Please try again.":"An unexpected error occurred. Please try again."}const Be="https://v2.api.noroff.dev";async function x(e,t={}){try{const n=fe(),o={"Content-Type":"application/json","X-Noroff-API-Key":"90d9d08a-808b-4c1f-8475-b0584204612f",...t.headers};n&&(o.Authorization=`Bearer ${n}`);const a=await fetch(`${Be}${e}`,{...t,headers:o});let r=null;const l=await a.text();if(r=l?JSON.parse(l):{},!a.ok)throw new Error(r.errors?.[0]?.message||"API error");return r}catch(n){console.error("API Client Error:",n.message);const o=$e(n);throw new Error(o)}}function T(){const e=z(),t=F();document.querySelectorAll('[data-auth="logged-in"]').forEach(n=>{n.classList.toggle("hidden",!e)}),document.querySelectorAll('[data-auth="logged-out"]').forEach(n=>{n.classList.toggle("hidden",e)}),document.querySelectorAll('[data-auth="username"]').forEach(n=>{n.textContent=t?t.username:""})}async function Ae(e){try{const t=await fetch("https://v2.api.noroff.dev/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)}),n=await t.json();if(!t.ok)throw new Error(n.error||"Failed to register user");return n.data}catch(t){throw new Error(`Registration error: ${t.message}`)}}async function Te(e){const n=(await x("/auth/login",{method:"POST",body:JSON.stringify(e)})).data;return pe(n),T(),n}function Ue(){ke(),localStorage.removeItem("apiKey"),T()}function ne(e){return e.length>=8&&/[A-Za-z]/.test(e)&&/[0-9]/.test(e)&&/[^A-Za-z0-9]/.test(e)}function oe(e){return e?/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)&&e.toLowerCase().endsWith("@stud.noroff.no"):!1}function ae(e){return/^[A-Za-zÆØÅæøå\s'-]{2,50}$/.test(e.trim())}function G(e,t){t?(e.classList.remove("text-red-500"),e.classList.add("text-green-500")):(e.classList.remove("text-green-500"),e.classList.add("text-red-500"))}function Fe(e){if(!e)return!0;const t=["images.unsplash.com","cdn.pixabay.com","i.imgur.com","images.pexels.com"],n=["porn","sex","nude","nsfw","xxx","adult","violence","gambling","drugs","hate","racism","terrorism"];try{const o=new URL(e);return!(!t.includes(o.hostname)||n.some(a=>e.toLowerCase().includes(a)))}catch(o){return console.error(o),!1}}function qe(){return`
  ...
  `}function Me(){const e=document.getElementById("newuser-form"),t=document.getElementById("form-message");if(!e||!t)return;const n=e.querySelector('input[name="name"]'),o=e.querySelector('input[name="email"]'),a=e.querySelector('input[name="password"]'),r=document.getElementById("email-rule"),l=document.getElementById("password-rule");n.addEventListener("input",()=>{G(r,ae(n.value))}),o.addEventListener("input",()=>{G(r,oe(o.value))}),a.addEventListener("input",()=>{G(l,ne(a.value))}),e.addEventListener("submit",async s=>{s.preventDefault();const i=new FormData(e),u=i.get("password"),p=i.get("email"),h=i.get("name");if(!ae(h)){t.textContent="Please enter a valid witch name.",t.className="text-red-500";return}if(!oe(p)){t.textContent="Please enter a valid email address.",t.className="text-red-500";return}if(!ne(u)){t.textContent="Password must be at least 8 characters long and include a number and a special character.",t.className="text-red-500";return}const N={name:i.get("name"),email:i.get("email"),password:i.get("password")};try{await Ae(N),t.textContent="Registration successful! Welcome to the Coven.",t.className="text-green-500",e.reset(),history.pushState(null,null,"/login"),I()}catch(w){t.textContent=w.message,t.className="text-red-500"}})}function ze(){return`
    <section class="p-6">
  ...
  `}document.title="Hex & Chill - Join the Coven";async function _e(e){return await x("/social/posts",{method:"POST",body:JSON.stringify(e)})}async function ge(e=""){return await x(`/social/posts?_author=true&_comments=true&_sort=created&_order=desc${e}`)}async function he(e){return await x(`/social/posts/${e}?_author=true&_comments=true`)}async function ye(e){return await x(`/social/posts/${e}`,{method:"DELETE"})}async function xe(e,t){return await x(`/social/posts/${e}`,{method:"PUT",body:JSON.stringify(t)})}let re;function g(e,t="error",n=3e3){let o=document.getElementById("toast");o||(o=document.createElement("div"),o.id="toast",o.className="fixed bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded shadow-lg text-white text-sm z-50 opacity-0 transition-opacity duration-300",document.body.appendChild(o)),o.textContent=e,o.classList.remove("bg-green-500","bg-red-500","bg-yellow-500"),o.classList.add(t==="success"?"bg-green-500":t==="warning"?"bg-yellow-500":"bg-red-500"),o.style.opacity="1",clearTimeout(re),re=setTimeout(()=>{o.style.opacity="0"},n)}function He(){const e=document.querySelector("[data-create-post-form]");e&&e.addEventListener("submit",async t=>{t.preventDefault();const n=e.title.value.trim(),o=e.body.value.trim(),a=e.tags.value?e.tags.value.split(",").map(i=>i.trim()):[],r=e.image?.value.trim();if(r&&!Fe(r)){g("Please use a sage image from unsplash, pixabay, imgur or pexels.","error");return}const l={title:n,body:o,tags:a,media:r?{url:r,alt:n}:null},s=e.dataset.postId;try{s?await xe(s,l):await _e(l),document.dispatchEvent(new Event("post:updated")),history.pushState(null,null,"/"),I(),window.scrollTo(0,0)}catch(i){alert(i.message||"Failed to create post. Please try again.")}})}function Re(e={}){const t=document.createElement("section"),n=document.createElement("form");n.dataset.createPostForm="",n.dataset.postId=e.id||"",n.className="mt-20 max-w-md mx-auto p-6 bg-[var(--color-bg-surface)] border border-gray-700 rounded-lg shadow-sm";const o=document.createElement("input");o.type="text",o.name="title",o.value=e.title||"",o.placeholder="Spell Title",o.className="w-full p-2 mb-4 rounded bg-gray-800 text-sm font-[Inter]",o.required=!0;const a=document.createElement("textarea");a.name="body",a.placeholder="Your spellbinding thoughts...",a.className="w-full p-2 mb-4 rounded bg-gray-800 text-sm font-[Inter]",a.required=!0,a.value=e.body||"";const r=document.createElement("input");r.type="text",r.name="tags",r.value=e.tags?e.tags.join(", "):"",r.placeholder="tags (comma separated)",r.className="w-full p-2 mb-4 rounded bg-gray-800 text-sm font-[Inter]";const l=document.createElement("input");l.type="url",l.name="image",l.value=e.media?.url||"",l.placeholder="Image url from safe sources like pexels, unsplash, etc...",l.className="w-full p-2 mb-4 rounded bg-gray-800 text-sm font-[Inter]";const s=document.createElement("button");return s.type="submit",s.textContent=e.id?"Update Spell":"Cast Spell",s.className="w-full py-2 rounded bg-purple-400 hover:bg-purple-500 transition font-[Cinzel] text-white",n.appendChild(o),n.appendChild(a),n.appendChild(r),n.appendChild(l),n.appendChild(s),t.appendChild(n),t}function De(){return`
    <div
      id="login-modal"
  ...
  `,We=`
    ...
    transition
    hover:shadow-[0_0_12px_rgba(199,125,255,0.4)]
  `;let O=0;const le=6;let J=!1,M=!1;async function Ee(e=!1){if(M||J)return;M=!0;const t=document.getElementById("load-more-posts");t&&(t.textContent="Loading...");try{const o=(await ge(`&page=${O}&limit=${le}`)).data;if(!o.length){t?.remove(),M=!1;return}await be(o,{append:e}),o.length<le?(J=!0,t?.remove()):O++}catch(n){console.error("Failed to load more posts:",n),g("Failed to load more posts. Please try again.","error")}t&&(t.textContent="Load More"),M=!1}function Ke(){if(document.getElementById("load-more-posts"))return;const e=document.getElementById("app"),t=document.createElement("button");t.id="load-more-posts",t.textContent="Load More",t.className=`
    mt-6 px-4 py-2 rounded-md border border-gray-300/60 hover:border-purple-400 transition
    ${Ce}
    bg-transparent
    mx-auto mb-10
  `,t.addEventListener("click",()=>Ee(!0)),e.appendChild(t)}async function Ye(){const e=document.getElementById("posts");if(e){if(!z()){O=1,J=!1,M=!1,e.innerHTML=`   <section class="p-6 space-y-6">
      <div>
      <img
...
Explore the latest posts from our coven, discover new profiles, and let your magic be known.
 ...
 </section>`,document.getElementById("load-more-posts")?.remove();return}e.innerHTML="",O=1,J=!1;try{const t=F();if(t){const o=await x(`/social/profiles/${t.username}?_following=true`);X(o.data.following||[])}const[,n]=await Promise.all([Ee(!1),j()]);return Ke(),{profiles:n.data}}catch(t){console.error("Failed to fetch posts:",t),t.message.includes("API key")?g("Please log out and log back in to refresh your API key","error"):g("Failed to load posts. Please try again.","error")}}}function Ge(){return`
    <section class="p-6">
    <h1 class="text-2xl font-[Cinzel] text-purple-400 mb-4 text-center">
      Coven Profiles
    </h1>
    ...
    `}document.title="Hex & Chill - Coven Profiles";function V(e=""){return String(e).replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"})[n])}async function Ve(e){const t=document.getElementById("profiles-container");if(!t){console.warn("Profiles container not found");return}if(!e||e.length===0){const n=document.createElement("p");n.className="text-center text-gray-500",n.textContent="No profiles found.",t.appendChild(n);return}t.innerHTML=await Promise.all(e.map(async n=>`
    <div class="bg-white rounded-lg shadow p-4">
      <img 
        src="${n.avatar?.url||await W()}" 
        alt="${V(n.name||"Unknown")}"
        class="w-16 h-16 rounded-full mx-auto mb-2"
      />
      <h3 class="text-center font-bold text-purple-600">${V(n.name||"Unknown")}</h3>
      <p class="text-center text-sm text-gray-600">${V(n.bio||"No bio available")}</p>
      <div class="flex justify-center gap-4 mt-2 text-sm text-gray-500">
        <span>Posts: ${n._count?.posts||0}</span>
        <span>Followers: ${n._count?.followers||0}</span>
      </div>
    </div>
  `).join(""))}async function Ze(){if(!z()){g("You must be logged in to view profiles.");return}try{const e=await j();await Ve(e.data)}catch(e){console.error("Failed to load profiles:",e.message),g("Failed to load profiles. Please try again later.")}}function Xe(e){return`
  ...
  `}async function Qe(e){try{const[t,n]=await Promise.all([we(e,"?_posts=true$_followers=true&following=true"),ve(e)]),o=t.data,a=n.data;document.getElementById("profile-detail").innerHTML=`
        ...
        `;const r=document.getElementById("change-avatar");if(!r){console.warn("Change avatar button not found");return}r.addEventListener("click",async()=>{const s=prompt("Enter new avatar URL: ");if(s)try{await Je(e.username,{avatar:{url:s,alt:`${o.name}'s avatar`}}),g("Avatar updated successfully!","success"),document.querySelector("#own-profile-info img").src=s}catch(i){console.error("Failed to update avatar:",i),g("Failed to update avatar. Please try again.","error")}});const l=document.getElementById("bio-input");l.value=o.bio||"",await be(a,{containerId:"own-posts"})}catch(t){console.error("Error loading profile:",t),g("Failed to load profile. Please try again later.","error")}}function ot(){return`
  ...
  `}async function rt(e){if(!e||isNaN(e)){console.error("Invalid post ID:",e);return}try{const o=(await he(e)).data;document.querySelector('[name="title"]').value=o.title,document.querySelector('[name="body"]').value=o.body,document.querySelector('[name="tags"]').value=(o.tags||[]).join(", "),document.querySelector('[name="image"]').value=o.media?o.media.url:""}catch(n){console.error("Failed to load post:",n.message),g("Failed to load post. Please try again later.");return}const t=document.querySelector("[data-edit-post-form]");t&&t.addEventListener("submit",async n=>{n.preventDefault();const o=t.querySelector('[name="title"]').value.trim(),a=t.querySelector('[name="body"]').value.trim(),r=t.querySelector('[name="tags"]').value.split(",").map(s=>s.trim()).filter(s=>s),l=t.querySelector('[name="image"]').value.trim();if(!o||!a){g("Title and body are required.","error");return}try{await xe(e,{title:o,body:a,tags:r,media:l?{url:l}:null}),g("Post updated successfully!","success"),history.pushState(null,null,`/post/${e}`),I()}catch(s){console.error("Failed to update post:",s),g("Failed to update post. Please try again.","error")}})}async function st(e=[]){const t=document.createElement("div"),n=document.createElement("h1");n.className="text-2xl font-[Cinzel] font-semibold text-purple-400 mb-6 text-center mt-4",n.textContent="My Familiars";const o=document.createElement("section");o.className="grid grid-cols-4 items-center gap-4 px-6 py-4 overflow-x-auto border-b border-gray-700 bg-[#121212] sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4";for(const a of e){const r=a.avatar?.url||await W(),l=document.createElement("a");l.href=`/profiles/${a.name}`,l.dataset.link="",l.className="block";const s=document.createElement("div");s.className="flex flex-col items-center gap-3 shrink-0";const i=document.createElement("div");i.className="w-24 h-24 rounded-full border-2 border-gray-300/70 p-0.5 hover:border-purple-400 transition mb-4 mt-6";const u=document.createElement("img");u.src=r,u.alt=`${a.name} avatar`,u.className="w-full h-full rounded-full object-cover";const p=document.createElement("span");p.className="text-xs font-[Cinzel] text-gray-300",p.textContent=a.name,i.appendChild(u),s.appendChild(i),s.appendChild(p),l.appendChild(p),l.appendChild(s),o.appendChild(l)}return t.appendChild(n),t.appendChild(o),t}async function lt(){const e=document.getElementById("app"),t=U();e.innerHTML="";const n=await st(t);e.appendChild(n)}async function ie(e){const t=U(),n=new Set(t.map(r=>r.name)),o=e.filter(r=>n.has(r.name));return(await Promise.all(o.map(async r=>{if(r.avatar?.url)return r;const l=await W();return{...r,avatar:{url:l,alt:`${r.name} avatar`}}}))).slice(0,6)}function it(){const e=document.createElement("div");e.id="search-overlay",e.className="fixed inset-0 bg-black/60 backdrop-blur-sm hidden z-50 flex items-start justify-center pt-32";const t=document.createElement("div");t.className="w-full max-w-xl bg-[#2b023c] rounded-xl border border-purple-700 ";const n=document.createElement("input");n.id="search-input",n.type="text",n.placeholder="Scry the Coven...",n.className="w-full p-3 rounded bg-purple-600 text-white outline-none";const o=document.createElement("div");return o.id="search-results",o.className="mt-4 space-y-2",t.appendChild(n),t.appendChild(o),e.appendChild(t),e}const ct={"/":de,"/newuser":Oe(ze),"/profiles":Ge,"/createPost":Re};async function I(){const e="/JS2_CA";let t=window.location.pathname;if(t.startsWith(e)&&(t=t.slice(e.length)||"/"),document.getElementById("sidenav").innerHTML=Ne(),document.getElementById("header").innerHTML=Pe(),document.getElementById("search-overlay")||document.body.appendChild(it()),t.startsWith("/profiles/")&&t!=="/profiles"){const s=t.split("/")[2];document.getElementById("app").innerHTML=Xe(s);let u=(await j()).data;u=await ie(u);const p=document.getElementById("sidenavRight");p.innerHTML="";const h=await te();p.appendChild(h),Z(),T(),await Qe(s);return}const n=ct[t]||de,o=document.getElementById("app");o.innerHTML="";const a=n();if(a instanceof HTMLElement?o.appendChild(a):o.innerHTML=a,z()){const s=F();if(s)try{const i=await x(`/social/profiles/${s.username}?_following=true`);X(i.data.following||[])}catch(i){console.warn("Could not fetch following list:",i.message)}}let r=[];t==="/"?r=(await Ye())?.profiles||[]:r=(await j()).data,r=await ie(r);const l=document.getElementById("sidenavRight");if(l.innerHTML="",l.appendChild(await te()),Z(),T(),t==="/newuser"&&Me(),t==="/profiles"&&Ze(),t==="/createPost"&&He(),t.startsWith("/post/")){const s=t.split("/post/")[1];tt(s);return}if(t==="/ownProfile"&&(document.getElementById("app").innerHTML=ot(),nt()),t.startsWith("/edit-post")){const s=t.split("/edit-post/")[1];document.getElementById("app").innerHTML=at(),await rt(s);return}if(t==="/familiars"){await lt();return}}function dt(){const e=localStorage.getItem("authUser");if(!e)return null;try{return JSON.parse(e)}catch{return null}}async function ut(e){return await x(`/social/profiles/${e}/follow`,{method:"PUT"})}async function mt(e){return await x(`/social/profiles/${e}/unfollow`,{method:"PUT"})}function pt(){document.addEventListener("click",async e=>{const t=e.target.closest('button[data-auth="follow"]'),n=e.target.closest('button[data-auth="followed"]');if(!t&&!n)return;const a=(t||n).dataset.username;try{let r;t?r=await ut(a):r=await mt(a),X(r.data.following),ft(a)}catch(r){console.error("Error updating follow status:",r),alert("Failed to update follow status. Please try again.")}})}function ft(e){const n=U().some(a=>a.name===e);document.querySelectorAll("[data-follow-container]").forEach(a=>{const r=a.querySelector('[data-auth="follow"]'),l=a.querySelector('[data-auth="followed"]');!r||!l||r.dataset.username===e&&(r.classList.toggle("hidden",n),l.classList.toggle("hidden",!n))})}function gt(){const e=document.getElementById("search-overlay"),t=document.getElementById("search-input"),n=document.getElementById("search-results");!e||!t||!n||(document.addEventListener("click",o=>{o.target.closest("[data-open-search]")&&(e.classList.remove("hidden"),t.focus())}),e.addEventListener("click",o=>{o.target===e&&e.classList.add("hidden")}),window.addEventListener("keydown",o=>{o.key==="Escape"&&e.classList.add("hidden")}),n.addEventListener("click",o=>{o.target.closest("a[data-link]")&&e.classList.add("hidden")}),t.addEventListener("input",async()=>{const o=t.value.trim();if(!o){n.innerHTML="";return}if(o.length<2){n.innerHTML="";return}try{const l=((await ge(`&search=${encodeURIComponent(o)}`)).data||[]).filter(s=>{const i=s.title?.toLowerCase()||"",u=s.body?.toLowerCase()||"";return i.includes(o.toLowerCase())||u.includes(o.toLowerCase())});n.innerHTML=l.map(s=>`
